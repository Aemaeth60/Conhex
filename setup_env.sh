...
if [ ${DL_ENV}=="pytorch" ]; then
    ...
    docker run --runtime=nvidia --shm-size 8G -v `pwd`:/workspace -p ${NOTEBOOK_PORT}:8888 -p ${VISDOM_PORT}:8097 --name stepankuzmin/pytorch-notebook ${PYTORCH_IMAGE}
    docker exec stepankuzmin/pytorch-notebook jupyter notebook list
else
    exit 1
fi
